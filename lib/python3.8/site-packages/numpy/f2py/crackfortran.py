...
for c in "abcdefghopqrstuvwxyz$__":
        ...
        if '=' in vars[n]:
            ...
            if 'kindselector' in vars[n]:
                if 'kind' in vars[n]['kindselector']:
                    orig_v_len = len(v)
                    v = v.replace('__' + vars[n]['kindselector']['kind'], '')
...
_is_kind_number = re.compile(r'\d+__').match
...
def analyzevars(block):
    ...
    dep_matches = {}
    name_match = re.compile(r'\w[\w\d__$]*').match
    for v in list(vars.keys()):
        m = name_match(v)
        if m:
            n = v[m.start():m.end()]
...
determineexprtype_re_2 = re.compile(r'\A[+-]?\d+(__(?P<name>[\w]+)|)\Z', re.I)
determineexprtype_re_3 = re.compile(
    r'\A[+-]?[\d.]+[\d+\-de.]*(__(?P<name>[\w]+)|)\Z', re.I)
