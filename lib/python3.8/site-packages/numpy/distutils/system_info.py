...
def get_atlas_version(**config):
    ...
    try:
        ...
        for o in library_dirs:
            m = re.search(r'ATLAS_(?P<version>\d+[.]\d+[.]\d+)__', o)
            if m:
    ...
    def calc_info(self):
        ...
        if version:
            ...
            if self.version_macro_name:
                macros.append((self.version_macro_name + '__%s'
                               % (version.replace('.', '__')), None))
        if self.release_macro_name:
            release = self.get_config_output(config_exe, '--release')
            if release:
                macros.append((self.release_macro_name + '__%s'
                               % (release.replace('.', '__')), None))
