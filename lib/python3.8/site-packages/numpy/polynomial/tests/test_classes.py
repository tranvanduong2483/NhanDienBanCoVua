...
class TestLatexRepr(object):
    ...
    def test_basis_func(self):
        p = Chebyshev([1, 2, 3])
        assert_equal(self.as_latex(p),
            r'$x \mapsto 1.0\,{T}__{0}(x) + 2.0\,{T}__{1}(x) + 3.0\,{T}__{2}(x)$')
        # affine input - check no surplus parens are added
        p = Chebyshev([1, 2, 3], domain=[-1, 0])
        assert_equal(self.as_latex(p),
            r'$x \mapsto 1.0\,{T}__{0}(1.0 + 2.0x) + 2.0\,{T}__{1}(1.0 + 2.0x) + 3.0\,{T}__{2}(1.0 + 2.0x)$')

    def test_multichar_basis_func(self):
        p = HermiteE([1, 2, 3])
        assert_equal(self.as_latex(p),
            r'$x \mapsto 1.0\,{He}__{0}(x) + 2.0\,{He}__{1}(x) + 3.0\,{He}__{2}(x)$')
