...
def correlate(a, v, mode='valid'):
    ...
    may be defined differently. Another common definition is::
        ...
        c'__{av}[k] = sum_n a[n] conj(v[n+k])

    which is related to ``c_{av}[k]`` by ``c'__{av}[k] = c_{av}[-k]``.
...
def convolve(a, v, mode='full'):
    ...
    models the effect of a linear time-invariant system on a signal [1]__.  In
...
def binary_repr(num, width=None):
    ...
    representing signed integers on computers [1]__. A N-bit two's-complement
